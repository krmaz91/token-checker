...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/analyze") {
    ...
    if (isSolana && !isLikelySolanaMint(mint)) {
      return sendJson(res, 400, { error: "Please provide a valid Solana mint address." });
    }
    if (isEvm && !isLikelyEvmAddress(mint)) {
      return sendJson(res, 400, { error: "Please provide a valid EVM token address." });
    }
...
server.listen(PORT, () => {
  console.log(`Solana token analyzer running on http://localhost:${PORT}`);
});
